...
export function ClipPicker(props: {
  ...
  onOpenShelf: () => void
  // Whatever belongs to the clip this caption names rather than to the shelf
  // it came off — the ★ and the credit link, when the clip is a kept roll. The
  // same prop FileName takes, for the same reason and drawn in the same row: a
  // slot on the shelf plays other people's footage as readily as your own, and
  // the way through to the licence has to follow the picture rather than the
  ...
  return (
    <div className={styles.pickCaption}>
      <Popover
          ...
          <>
            ...
            <div className={styles.pickList}>
              {props.lib.clips.length === 0 ? (
                ...
                <div className={styles.pickEmpty}>
                  no clip matches “{query}”
                </div>
              ) : (
                shown.map(group => (
                  <div key={group.id}>
                    {/* The heading shows only when there is more than one thing to
                      tell apart. A single group's name over every row of a shelf
                      that is all one group is a line spent saying what the row
                      below it already says. */}
                    {shown.length === 1 ? null : (
                      <div className={styles.pickHead}>{group.label}</div>
                    )}
