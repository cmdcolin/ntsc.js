...
// motion amount, a knob on a preset weight, and a knob on an ordinary control
// (which must still take over softly). Nothing here needs hardware, which is the
// point — the alternative regression net for this feature is "plug a controller
// in and remember to try it", which is no net at all.
//
...
const armed = await page.evaluate(() => {
  const label = [...document.querySelectorAll('label')].find(
    l => l.textContent?.trim() === 'phosphor persistence',
  )
...
ok('a control row still offers its own ⚟', armed)
await new Promise(r => setTimeout(r, 200))
// The MIDI panel is hidden while the filter box has text in it (below the box
// is the result set), so the row's own button is what reports the bind here.
await page.evaluate(() => window.__cc(23, 40))
await new Promise(r => setTimeout(r, 400))
ok(
  ...
  await page.evaluate(() => {
    ...
    return [...(label?.closest('div')?.querySelectorAll('button') ?? [])].some(
      b => b.textContent?.trim() === 'CC23',
    )
  }),
)
