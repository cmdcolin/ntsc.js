...
const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${W} ${H}" width="${W}" height="${H}" role="img" aria-label="ntscsynth — one NTSC line of 75% color bars on a waveform monitor: horizontal sync, color burst, and the descending luma staircase with chroma subcarrier on each bar">
  <defs>
...
const markSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${S} ${S}" role="img" aria-label="ntscsynth — the 75% color bar staircase of an NTSC line">
